...
class _FocusModeScreenState extends State<FocusModeScreen> {
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setModalState) => Padding(
          ...
          child: Column(
            ...
            children: [
              Text('Focus Duration', style: Theme.of(context).textTheme.titleLarge),
              const SizedBox(height: 24),
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                children: [25, 50, 90].map((m) => ChoiceChip(
                  label: Text('$m min'),
                  selected: selectedMins == m,
                  onSelected: (s) => setModalState(() => selectedMins = m),
                )).toList(),
              ),
              const SizedBox(height: 24),
              Slider(
  ...
  void _showTimerComplete() {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text('Great work! You focused for ${_initialSeconds ~/ 60} minutes.'),
        actions: [
          ElevatedButton(
            onPressed: () {
              Navigator.pop(ctx);
  ...
  void _showStuckDialog() {
    ...
    final scriptedActions = state.habits.where((h) => h.type == HabitType.build).toList();
          ...
          children: [
            Row(
              children: [
                ...
                const SizedBox(width: 8),
                Text('You\'re Stuck?', style: Theme.of(context).textTheme.titleLarge),
              ],
            ),
            const SizedBox(height: 16),
            Text('Your Scripted Actions:', style: TextStyle(color: AppColors.textMuted)),
            ...
            if (scriptedActions.isEmpty)
              Container(
                ...
                child: Text('No scripted actions yet. Add them in the Habits screen!',
                    style: TextStyle(color: AppColors.textMuted)),
              )
            else
              ...scriptedActions.take(5).map((action) => Container(
                ...
                child: Row(
                  children: [
                    Text('✓', style: TextStyle(color: AppColors.success, fontWeight: FontWeight.bold)),
                    const SizedBox(width: 12),
                    Expanded(child: Text(action.name, style: TextStyle(color: AppColors.textPrimary))),
                  ],
                ),
              )),
            const SizedBox(height: 16),
            ...
            _QuickTip(icon: Icons.directions_walk_rounded, text: 'Stand up and stretch'),
            _QuickTip(icon: Icons.edit_note_rounded, text: 'Break into smaller steps'),
            const SizedBox(height: 16),
            SizedBox(
              width: double.infinity,
              child: ElevatedButton(
                  ...
                  child: Row(
                    children: [
                      ...
                      Container(
                        ...
                        child: Text('Focus Mode', style: TextStyle(color: AppColors.primary, fontWeight: FontWeight.w600)),
                      ),
                      const Spacer(),
                        ...
                        Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              Row(
                                children: [
                                  Text(widget.task.effortEmoji, style: const TextStyle(fontSize: 18)),
                                  const SizedBox(width: 8),
                                  Text(widget.task.impactEmoji, style: const TextStyle(fontSize: 18)),
                                  const Spacer(),
                                  if (subtasks.isNotEmpty)
                                    Text('$completedSubtasks/${subtasks.length}', style: TextStyle(color: AppColors.textMuted)),
                                ],
                              ),
                              const SizedBox(height: 8),
                              Text(
                              ...
                              if (widget.task.description.isNotEmpty) ...[
                                ...
                                Text(widget.task.description, style: TextStyle(color: AppColors.textMuted)),
                              ],
                            ],
                          ),
                        ),
                            ...
                            children: [
                              Row(
                                children: [
                                  ...
                                  const SizedBox(width: 8),
                                  Text('Distraction Pad', style: TextStyle(fontWeight: FontWeight.w600, color: AppColors.textPrimary)),
                                ],
                              ),
                              const SizedBox(height: 4),
                              Text('Offload thoughts here → auto-adds to backlog', style: TextStyle(fontSize: 11, color: AppColors.textMuted)),
                              const SizedBox(height: 12),
                              Row(
                                children: [
                                  Expanded(
                                  ...
                                  const SizedBox(width: 8),
                                  IconButton(
                                    icon: Icon(Icons.add_rounded, color: AppColors.warning),
                              ...
                              if (_distractions.isNotEmpty) ...[
                                ...
                                Wrap(
                                  spacing: 6,
                                  runSpacing: 6,
                                  children: _distractions.map((d) => Chip(
                                    label: Text(d, style: TextStyle(fontSize: 11, color: AppColors.textMuted)),
                    ...
                    children: [
                      Expanded(
                        child: OutlinedButton.icon(
                          ...
                          icon: Icon(Icons.help_outline_rounded, color: AppColors.warning),
                          label: Text('I\'m Stuck', style: TextStyle(color: AppColors.warning)),
...
class _TimerButton extends StatelessWidget {
  ...
  const _TimerButton({required this.icon, required this.color, required this.onTap});
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: Container(
        ...
        padding: const EdgeInsets.all(12),
        decoration: BoxDecoration(
          color: subtask.isCompleted ? AppColors.success.withAlpha(20) : AppColors.surfaceLight,
          borderRadius: BorderRadius.circular(12),
          border: Border.all(color: subtask.isCompleted ? AppColors.success.withAlpha(50) : AppColors.glassBorder),
        ),
        child: Row(
          children: [
            Icon(
